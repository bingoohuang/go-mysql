...
import (
	...
	"github.com/bingoohuang/go-mysql/dump"
...
func (c *Canal) run() error {
	...
	if !c.dumped {
		...
		if err != nil {
			log.Printf("E! canal dump mysql err: %v", err)
			return errors.Trace(err)
		}
	}
	...
	if err := c.runSyncBinlog(); err != nil {
		if errors.Cause(err) != context.Canceled {
			log.Printf("E! canal start sync binlog err: %v", err)
			return errors.Trace(err)
		}
	}
...
func (c *Canal) GetTable(db string, table string) (*schema.Table, error) {
	...
	if err != nil {
		...
		if c.cfg.DiscardNoMetaRowEvent {
			...
			log.Printf("E! canal get table meta err: %v", errors.Trace(err))
			return nil, schema.ErrMissingTableMeta
		}
		return nil, err
	}
